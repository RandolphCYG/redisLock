...
func (lock *RedisLock) TryLock() (bool, error) {
	success, err := lock.redisClient.SetNX(ctx, lock.key, lock.value, lock.expiration).Result()
	if err != nil {
		return false, err
	}
...
func (lock *RedisLock) Unlock() error {
	script := redis.NewScript(fmt.Sprintf(
		...
		lock.value))
	runCmd := script.Run(ctx, lock.redisClient, []string{lock.key})
...
func (lock *RedisLock) subscribeLock() error {
	pubSub := lock.redisClient.Subscribe(ctx, getPubSubTopic(lock.key))
...
func (lock *RedisLock) subscribeLockWithTimeout(d time.Duration) error {
	timeNow := time.Now()
	pubSub := lock.redisClient.Subscribe(ctx, getPubSubTopic(lock.key))
	_, err := pubSub.ReceiveTimeout(ctx, d)
	if err != nil {
		return err
	}
...
func (lock *RedisLock) publishLock() error {
	err := lock.redisClient.Publish(ctx, getPubSubTopic(lock.key), "release lock").Err()
	if err != nil {
		return err
	}
	return nil
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				lock.redisClient.Expire(ctx, lock.key, lock.expiration).Result()
			}
		}
	}()
}
